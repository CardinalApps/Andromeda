 ...
 */
 export class Bridge {
   ...
   */
  async _initIpc() {
    if (typeof require !== 'function') throw new Error('Cannot establish IPC connection because `require` is not function')

    try {
      this.electronIpc = require('electron').ipcRenderer
    } catch (error) {
      throw new Error(`Cannot load Electron's ipcRenderer module`)
    }
  ...
  async _initWs(options) {
    ...
    const connect = (cb) => {
      this.BridgeWebSocket = new WebSocket(`${wsBaseUrl}/bridge`)
   ...
   * API. Must start with a leading slash.
   * @param {string} verb - HTTP request method.
   * @param {*} body - Request body. Will be automatically stringified if necessary.
  ...
  httpApi(route, verb = 'GET', body = null) {
    return new Promise((resolve, reject) => {
      ...
      req.addEventListener('load', function loaded(progEvent) {
        ...
        // the right way to check if a string is JSON is to try and parse it
        try {
